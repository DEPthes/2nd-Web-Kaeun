...
const GamePage = () => {
  ...
  const [remaining, setRemaining] = useState(location.state?.num); //남은 개수
  const [count, setCount] = useState(0); //총 시도 횟수
  const [flippedCards, setFlippedCards] = useState([]); //뒤집힌 카드
  const [copyArray, setCopyArray] = useState(location.state?.array); //카드 배열

  const handleCardFlip = (index, value) => {
    if (flippedCards.length === 0) {
      //뒤집힌 카드가 없으면 첫 번째 카드 선택
      setFlippedCards([{ index, value }]); //flippedCards에 첫 번째 카드 저장
    } else if (flippedCards.length === 1) {
      //뒤집힌 카드가 하나 있으면 두 번째 카드 선택
      const firstCard = flippedCards[0];
      if (firstCard.index !== index) {
        //첫 번째 선택한 카드와 두 번째 선택한 카드가 다른 경우
        setFlippedCards((prevCards) => [...prevCards, { index, value }]); //flippedCards에 두 번째 카드 저장
        setCount((prevCount) => prevCount + 1); //총 시도 횟수 1 증가
        setTimeout(() => {
          //1초 후에 카드들을 다시 뒤집음
          const secondCard = { index, value };
